...
initStartEventPropertyValueSelect = ->
    ...
    $('#start-event-property-value-select').on 'change', (e, selectedValue) ->
        value = $(@).val()
        filter =
            property: 'featureid'
            operator: 'equals'
...
getResults = (from, to, eventCount, startEvent, endEvent, propertyFilters, excludedValues = null) ->
    ...
    Query.getFlowsResponse from, to, eventCount, startEvent, endEvent, propertyFilters, excludedValues
    .then (results) ->
...
runQueryWithExcludedValues = (excludedValues) ->
